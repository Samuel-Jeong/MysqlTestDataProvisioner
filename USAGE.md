...
2. 프로그램이 DB를 확인하고:
   - 존재하는 테이블: 스킵
   - 없는 테이블: CREATE TABLE 실행
3. 결과가 표시됩니다
   ```
   스키마 생성 완료: 생성 6개, 기존 0개, 오류 0개
   ```
...
2. 프로그램이 mysqldump를 실행하여 SQL 파일을 생성합니다
   ```
   mysqldump 완료: dump/dump_example_db_20231209153045.sql
   ```
...
### 예제 1: 개발 환경 데이터 생성 (처음 사용)

```
목표: 개발용 소량 테스트 데이터 생성 (DB에 테이블 없음)

1. 프로그램 실행
2. 메뉴 1: "example" 프로파일 선택
3. 메뉴 2: "small-test.json" 선택
4. 메뉴 3: 분석 실행
5. 메뉴 4: 스키마 생성 (테이블 생성)
   → 6개 테이블 생성됨
6. 메뉴 5: 데이터 생성
   → 고객 5명, 제품 10개, 주문 8개 등 생성됨
```

### 예제 2: 성능 테스트 데이터 생성 (테이블 있음)

```
목표: 대량 데이터로 성능 테스트 (이미 테이블 존재)

1. 프로그램 실행
2. 메뉴 1: "example" 프로파일 선택
3. 메뉴 2: "large-test.json" 선택
4. 메뉴 3: 분석 실행
5. 메뉴 4: 스키마 생성 (선택) - 이미 있으면 스킵됨
6. 메뉴 5: 데이터 생성
