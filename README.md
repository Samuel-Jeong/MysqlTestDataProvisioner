...
2. **시나리오 기반 데이터 생성** - 연관관계 매핑 및 테스트 레코드 자동 생성
3. **데이터 롤백** - 생성된 데이터를 run 단위로 삭제
4. **MySQL 덤프** - mysqldump를 통한 백업
5. **스키마 보호** - DDL 변경 없이 레코드만 조작
6. **중복 실행 방지** - 락 파일 기반
7. **TUI 인터페이스** - 직관적인 터미널 UI
...
4: 테스트 데이터 생성  - 실제 DB에 데이터 INSERT
5: 데이터 롤백      - 이전에 생성한 데이터 삭제
6: MySQL 덤프       - DB 전체 백업
Q: 종료
```
